...
def test_card_instance_creation_success():
    ...
    assert instance.card_holder_id == holder_id
...
def test_card_instance_validations():
    ...
    with pytest.raises(ValueError, match="associado a uma família"):
        CardInstance(family_id=None, credit_card_id=card_id, card_holder_id=holder_id, nickname="Físico")

    with pytest.raises(ValueError, match="associado a um contrato de cartão"):
        CardInstance(family_id=fam_id, credit_card_id=None, card_holder_id=holder_id, nickname="Físico")

    with pytest.raises(ValueError, match="deve ter um portador"):
        CardInstance(family_id=fam_id, credit_card_id=card_id, card_holder_id=None, nickname="Físico")
