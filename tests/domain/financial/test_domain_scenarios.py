...
@fixture
def family() -> Family:
    return Family(name="Família Teste")

@fixture
def member(family: Family) -> Member:
    return Member(family_id=family.id, name="Membro Teste 1")

@fixture
def bank(family: Family, member: Member) -> BankAccount:
    return BankAccount(family_id=family.id, holder_id=member.id, nickname="Itaú", bank="Itaú")

@fixture
def credit_card(family: Family, member: Member) -> CreditCard:
    return CreditCard(family_id=family.id, holder_id=member.id, nickname="Nubank",
        brand="Mastercard", issuer="Nubank", due_day=10, limit=Decimal('5000'))

@fixture
def accounts_payable(family: Family, member: Member) -> AccountsPayable:
    return AccountsPayable(family_id=family.id, holder_id=member.id, nickname="Boletos")

@fixture
def category_income() -> Category:
    return Category(name="Salário", type=TransactionType.INCOME)

@fixture
def category_expense() -> Category:
    return Category(name="Despesa Geral", type=TransactionType.EXPENSE)

@fixture
def category_fees() -> Category:
    return Category(name="Juros e Multas", type=TransactionType.EXPENSE)

def test_scenario_1_setup_and_heterogeneous_income(family: Family, bank: BankAccount, category_income: Category, category_expense: Category):
    
    # Renda 1 (Dia 5)
    tx_inc1 = Transaction(
        family_id=family.id, account_id=bank.id, category_id=category_income.id,
    ...
    tx_exp1 = Transaction(
        family_id=family.id, account_id=bank.id, category_id=category_expense.id,
...
def test_scenario_2_deferred_expense_and_clearing(family, member, bank, credit_card, accounts_payable,
    category_income, category_expense, category_fees):
    
    # Boleto gerado (Fato gerador em Julho, Vencimento 10 de Agosto)
    tx_boleto = Transaction(
        family_id=family.id, account_id=accounts_payable.id, category_id=category_expense.id,
    ...
    mb_jul_ap = MonthlyBalance.create_from_history(
        account_id=accounts_payable.id, month="2026-07",
        previous_real_balance=Decimal('0.00'), previous_projected_balance=Decimal('0.00'),
    ...
    tx_multa = Transaction(
        family_id=family.id, account_id=accounts_payable.id, category_id=category_fees.id,
    ...
    pagamento = Transfer(
        family_id=family.id, source_account_id=bank.id, destination_account_id=accounts_payable.id,
        amount=Decimal('1050.00'), date=date(2026, 8, 15), description="Pagamento Condomínio"
    )
    out_tx, in_tx = pagamento.generate_transactions(category_id=category_expense.id)
    ...
    mb_aug_ap = MonthlyBalance.create_from_history(
        account_id=accounts_payable.id, month="2026-08",
...
def test_scenario_3_credit_card_installments(family, member, bank, credit_card, accounts_payable, category_income,
    category_expense, category_fees):
    ...
    for i in range(1, 7):
        installments.append(
            Transaction(
                family_id=family.id, account_id=credit_card.id, category_id=category_expense.id,
    ...
    mb_jan_cc = MonthlyBalance.create_from_history(
        account_id=credit_card.id, month="2026-01",
        previous_real_balance=Decimal('0.00'), previous_projected_balance=Decimal('0.00'),
    ...
    for i in range(1, 4):
        pagamento = Transfer(
            family_id=family.id, source_account_id=bank.id, destination_account_id=credit_card.id,
            amount=Decimal('200.00'), date=date(2026, i, 10), description=f"Pagto Fatura {i}"
        )
        _, in_tx = pagamento.generate_transactions(category_id=category_expense.id)
    ...
    mb_mar_cc = MonthlyBalance.create_from_history(
        account_id=credit_card.id, month="2026-03",
        previous_real_balance=Decimal('0.00'), previous_projected_balance=Decimal('0.00'),
        transactions=installments + payments
    )
    assert mb_mar_cc.real_balance == Decimal('-600.00')

def test_scenario_4_indebtedness_and_rollover(family, member, bank, credit_card, accounts_payable,
    category_income, category_expense, category_fees):
    
    # --- MÊS 1 ---
    # Receitas: 3000. Despesas: 2000 (Débito) + 2000 (Cartão)
    tx_inc = Transaction(family_id=family.id, account_id=bank.id, category_id=category_income.id, type=TransactionType.INCOME, accrual_date=date(2026, 1, 5), due_date=date(2026, 1, 5), amount=Decimal('3000.00'), description="Salário")
    tx_exp_cash = Transaction(family_id=family.id, account_id=bank.id, category_id=category_expense.id, type=TransactionType.EXPENSE, accrual_date=date(2026, 1, 15), due_date=date(2026, 1, 15), amount=Decimal('2000.00'), description="Aluguel")
    tx_exp_cc = Transaction(family_id=family.id, account_id=credit_card.id, category_id=category_expense.id, type=TransactionType.EXPENSE, accrual_date=date(2026, 1, 20), due_date=date(2026, 2, 10), amount=Decimal('2000.00'), description="Móveis")
    ...
    mb_m1_cc = MonthlyBalance.create_from_history(account_id=credit_card.id, month="2026-01", previous_real_balance=Decimal('0.00'), previous_projected_balance=Decimal('0.00'), transactions=[tx_exp_cc])
    ...
    tx_inc2 = Transaction(family_id=family.id, account_id=bank.id, category_id=category_income.id, type=TransactionType.INCOME, accrual_date=date(2026, 2, 5), due_date=date(2026, 2, 5), amount=Decimal('3000.00'), description="Salário")
    
    pagamento_parcial = Transfer(family_id=family.id, source_account_id=bank.id, destination_account_id=credit_card.id, amount=Decimal('1000.00'), date=date(2026, 2, 10), description="Pagto Parcial Fatura")
    out_tx, in_tx = pagamento_parcial.generate_transactions(category_id=category_expense.id)
    
    tx_fees = Transaction(family_id=family.id, account_id=credit_card.id, category_id=category_fees.id, type=TransactionType.EXPENSE, accrual_date=date(2026, 2, 15), due_date=date(2026, 3, 10), amount=Decimal('100.00'), description="Juros Rolagem")
    tx_new_cc = Transaction(family_id=family.id, account_id=credit_card.id, category_id=category_expense.id, type=TransactionType.EXPENSE, accrual_date=date(2026, 2, 20), due_date=date(2026, 3, 10), amount=Decimal('500.00'), description="Roupas")
    ...
    mb_m2_cc = MonthlyBalance.create_from_history(account_id=credit_card.id, month="2026-02", previous_real_balance=mb_m1_cc.real_balance, previous_projected_balance=mb_m1_cc.projected_balance, transactions=[in_tx, tx_fees, tx_new_cc])
